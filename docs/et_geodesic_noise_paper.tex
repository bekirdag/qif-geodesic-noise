...
\item \textbf{Injection efficiency:} $>90\%$ detection efficiency at a pre-defined $A_h$ threshold with $\gamma=2$, under both rank-1 and rank-2 environmental models; for threshold curves, sweep $A_h$ over orders of magnitude. \cite{r1,r3}
\item \textbf{Stability:} LR statistic must not shift by more than $10\%$ under alternative knot placements or bootstrap modes; when comparing different bin counts, report LR per bin. \cite{r1,r3}
\end{itemize}

\noindent\textbf{Interpretation note:} inspect bootstrap LR distributions (not just p-values) and use sufficient bootstrap counts for resolution. \cite{r3,r15}
\noindent\textbf{Nestedness check:} ensure the signal fit can reach $\alpha\approx 0$ (warm-start from the env fit); persistent negative LR indicates optimization or constraint issues. \cite{r3,r15}
